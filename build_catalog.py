...
Концепция 1 «Инженерный лист». Один файл: DATA-блок (JS-объект) + шаблоны рендерятся в браузере.
Владелец редактирует только DATA — вёрстка сама подстраивается.
Этап 3: каркас + раздел «Для дачи и участка» целиком. Правки штаба (второй раунд) применены.
"""
import json
...
DATA = {
    ...
    "dopolnitelno": ("Дополнительно комплектуем электрикой, утеплением 100 мм, дополнительными окнами "
                      "и дверями, перегородками, линолеумом, конвекторами, ступенями. "
                      "Стоимость назовём при расчёте."),
    "famNames": {
    ...
    "stepNote": "Не уверены — позвоните, подберём за пять минут.",
    "woodMetal": {
        "wood": {"title": "Дерево", "text": "Теплее на вид, дешевле вход. Для постоянного места "
                 "на участке — дача, куда вы не планируете переезжать."},
        "metal": {"title": "Металл", "text": "Сварной каркас, переносит многократные переезды между "
                  "объектами, не ведёт геометрию, можно ставить в два этажа. Служит около 20 лет."},
        "note": "Не уверены, что подойдёт, — позвоните, поможем выбрать.",
    },
    "pricier": [
        ["Приезжает готовым.",
         "Многие боятся не самой бытовки или бани, а месяца грязной стройки на участке: чужая "
         "бригада, доски и утеплитель до заморозков. У нас наоборот — строение собирают и проверяют "
         "на производстве, а к вам оно приезжает уже готовым. Привезли, поставили на блоки — и в тот "
         "же день можно заезжать или топить печь."],
        ["Цена настоящая.",
         "На рынке принято писать цену за самый маленький размер, а по факту она подрастает на "
         "доставку, отделку и сборку — узнаёте об этом уже после звонка. Мы называем цену за то, что "
         "реально входит в комплектацию, без «от» и без сюрпризов в конце разговора."],
        ["Договор и гарантия 12 месяцев.",
         "Переводить деньги незнакомой компании страшно, и это нормально. Договор фиксирует размер, "
         "комплектацию и цену до копейки, а на изделие действует гарантия 12 месяцев."],
        ["Материалы, за которые не стыдно.",
         "Внутри — вагонка класса А/В, а не класс С, которым обшивает большинство рынка. Утепляем "
         "весь контур — пол, стены и потолок. Кровля — оцинкованный лист с фальцевым соединением, "
         "а не что придётся."],
        ["Мы отвечаем быстро.",
         "Знакомо, когда продавец читает сообщение и молчит днями? У нас с первого сообщения говорит "
         "менеджер — пишите в чат, вопросы по расчёту, доставке и монтажу решаются там же."],
    ],
    "dividers": [
        {"accent": "dacha", "title": "Для дачи и участка",
         "tagline": "Дом или хозблок, который привозят собранным и ставят за один день."},
    ],
    "families": [
        {
            "num": 1, "slug": "dacha-bytovki", "accent": "dacha", "name": "Дачные бытовки",
            "audience": "Дача, куда ездят по выходным, — переодеться, попить чаю, сложить садовый "
                        "инвентарь, без стройки и лишних трат.",
            "differ": "От «Металлических для дачи» это семейство отличает более доступная цена — "
                      "каркас деревянный: собирается быстрее и дешевле, но требует чуть больше ухода.",
            "kit": [
                "Деревянный каркас на брусовом основании 150×100 мм",
                "Утепление минватой 50 мм — пол, стены, потолок",
                "Вагонка хвои класса А/В внутри (не ДВП) — тепло и опрятно, как в доме; наружная "
                "отделка на выбор — та же вагонка или оцинкованный профлист",
        ...
        {
            "num": 2, "slug": "dush-tualet", "accent": "dacha", "name": "Дачные с душем и туалетом",
            "audience": "Дача без централизованной воды, где хочется помыться после огорода и не "
                        "бегать в дачный туалет через весь участок.",
            "differ": "В отличие от обычных «Дачных бытовок» сюда уже встроен санузел — не приходится "
                      "отдельно ставить бак с душем или бегать в дачный туалет.",
            "kit": [
        ...
        {
            "num": 3, "slug": "loft", "accent": "dacha", "name": "ЛОФТ",
            ...
            "differ": "От «Металлических для дачи» это семейство отличают готовые ПВХ-окна и уже "
                      "разведённая электрика в цене — модуль под интерьер и работу, а не дачное жильё.",
            "kit": [
                ...
                "Электрика уже разведена: розетки, светильники, автомат, уличный свет — часть цены",
        ...
        {
            "num": 4, "slug": "stykovannye-dachnye", "accent": "dacha", "name": "Стыкованные дачные",
            "audience": "Семье, которой нужен не один отдельный модуль, а маленький дом с комнатами — "
                        "кухня, спальня, прихожая под одной крышей.",
            "differ": "В отличие от одиночной «Дачной бытовки» это уже дом с планировкой — прихожая "
                      "и отдельные комнаты вместо одного отсека.",
            "kit": [
        ...
        {
            "num": 5, "slug": "metall-dacha", "accent": "dacha", "name": "Металлические для дачи",
            "audience": "Дача на много лет вперёд или участок без присмотра большую часть сезона — "
                        "каркас не гниёт и не ведёт геометрию со временем.",
            "differ": "От «Блок-контейнеров Стандарт» отличие в том, что отделка вагонкой уже выбрана "
                      "за вас — специально для уютной дачи, а не рабочих задач стройки.",
            "kit": [
...
.screen{{ width:1080px; height:1920px; background:var(--surface); position:relative;
  overflow:hidden; box-shadow:0 2px 10px rgba(0,0,0,.15); flex-shrink:0; }}
...
# ── JS: рендер из DATA (никаких данных не зашито в разметку) ───────────
JS = r'''
const ICON_PHONE = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M6 3h3l1.5 4-2 1.4a11 11 0 0 0 5.1 5.1l1.4-2 4 1.5v3a2 2 0 0 1-2.2 2A16 16 0 0 1 4 5.2 2 2 0 0 1 6 3Z"/></svg>';
...
function buttonCluster() {
  ...
  return `<div class="cluster">
    <a class="btn-call" href="${DATA.brand.phoneTel}">${ICON_PHONE}<span>Позвонить</span></a>
    <div class="msgrs">
      <a class="msgr" href="${L.whatsapp}" target="_blank" rel="noopener"><img src="foto/icons/whatsapp.svg" alt="WhatsApp"></a>
      <a class="msgr" href="${L.telegram}" target="_blank" rel="noopener"><img src="foto/icons/telegram.svg" alt="Telegram"></a>
      <a class="msgr" href="${L.max}" target="_blank" rel="noopener"><img src="foto/icons/max.png" alt="Max"></a>
    </div>
  </div>`;
}
...
function footerBlock() {
  return `<div class="p-foot">
    ${buttonCluster()}
    <div class="foot-row">
      <span class="phone-no">${DATA.brand.phoneText} · ${DATA.brand.hours}</span>
      <a class="nav-up" href="#navigator">${ICON_UP}<span>к навигатору</span></a>
    </div>
  </div>`;
}

// accentKey=null -> нейтральная шапка входных экранов (бренд, без цветной метки раздела)
function headerBlock(accentKey, famNum) {
  if (!accentKey) {
    return `<div class="p-head">
      <div class="p-tag"><b>КОНСТРУКТИВНЫЕ РЕШЕНИЯ</b></div>
    </div>`;
  }
  const [color, label] = DATA.accents[accentKey];
  const idx = famNum ? `<div class="p-idx mono">${String(famNum).padStart(2,'0')} / ${DATA.totalFamilies}</div>` : '';
  return `<div class="p-head">
...
function renderTable(rows) {
  let t = '<table class="data-table"><thead><tr><th>Модель</th><th>Размер</th><th>Особенности</th><th>Цена</th></tr></thead><tbody>';
  for (const [name, size, feat, price] of rows) {
    t += `<tr><td>${name}</td><td>${size}</td><td>${feat}</td><td class="mono price">${fmtPrice(price)}</td></tr>`;
  }
  return t + '</tbody></table>';
}

function familyVisual(fam) {
  const [color] = DATA.accents[fam.accent];
  const s = fam.slug;
  let html = `<section class="screen" id="fam-${fam.num}-visual"><div class="stripe" style="background:${color}"></div><div class="p-pad">`;
...
function familyData(fam) {
  const [color] = DATA.accents[fam.accent];
  let html = `<section class="screen" id="fam-${fam.num}-data"><div class="stripe" style="background:${color}"></div><div class="p-pad">`;
  html += headerBlock(fam.accent, fam.num);
  html += `<div class="p-title small">${fam.name}</div>`;
  html += '<div class="p-sub">Размеры и цены</div>';
  const models = fam.models;
  if (models.length > 10) {
    const mid = Math.ceil(models.length / 2);
    html += `<div class="data-table-split">${renderTable(models.slice(0,mid))}${renderTable(models.slice(mid))}</div>`;
  } else {
    html += renderTable(models);
  }
  html += '<div class="kit-title">Что входит в цену</div>';
  html += '<ul class="kit-list">' + fam.kit.map(k => `<li>${k}</li>`).join('') + '</ul>';
  html += `<div class="dop-line">${DATA.dopolnitelno}</div>`;
  html += '<div class="p-spacer"></div>';
  html += footerBlock();
  html += '</div></section>';
  return html;
...
function dividerScreen(d) {
  const [color] = DATA.accents[d.accent];
  return `<section class="screen divider-screen" id="razdel-${d.accent}">
    <div class="divider-inner" style="background:${color}">
      <div class="divider-title">${d.title}</div>
      <div class="divider-tag">${d.tagline}</div>
    </div>
  </section>`;
}
...
function screenCover() {
  const c = DATA.cover;
  let html = '<section class="screen" id="cover"><div class="p-pad cover-pad">';
  html += `<div class="cover-kicker">${c.kicker}</div>`;
  html += `<div class="cover-title">${c.title}</div>`;
  html += `<div class="cover-sub">${c.sub}</div>`;
  html += photoPh(c.photo.file, c.photo.w, c.photo.h);
  html += `<div class="cover-stat mono">${c.stat}</div>`;
  html += '<div class="p-spacer"></div>';
  html += footerBlock();
  html += '</div></section>';
  return html;
...
function screenThreeSteps() {
  let html = '<section class="screen" id="tri-shaga"><div class="p-pad">';
  html += headerBlock(null);
  html += '<div class="p-title small">Как выбрать за три шага</div>';
  for (const [num, title, text] of DATA.steps) {
    html += `<div class="step-row"><div class="step-num mono">${num}</div>
      <div><div class="step-title">${title}</div><div class="step-text">${text}</div></div></div>`;
  }
  html += `<div class="step-note">${DATA.stepNote}</div>`;
  html += '<div class="p-spacer"></div>';
  html += footerBlock();
  html += '</div></section>';
  return html;
...
function screenNavigator() {
  let html = '<section class="screen" id="navigator"><div class="p-pad">';
  html += headerBlock(null);
...
function screenWoodOrMetal() {
  const wm = DATA.woodMetal;
  let html = '<section class="screen" id="derevo-metall"><div class="p-pad">';
  html += headerBlock(null);
  html += '<div class="p-title small">Дерево или металл</div>';
  html += `<div class="two-col">
    <div class="col"><div class="col-title">${wm.wood.title}</div><div class="col-text">${wm.wood.text}</div></div>
    <div class="col"><div class="col-title">${wm.metal.title}</div><div class="col-text">${wm.metal.text}</div></div>
  </div>`;
  html += `<div class="step-note">${wm.note}</div>`;
  html += '<div class="p-spacer"></div>';
  html += footerBlock();
  html += '</div></section>';
  return html;
...
function screenWeArePricier() {
  let html = '<section class="screen" id="my-dorozhe"><div class="p-pad">';
  html += headerBlock(null);
  html += '<div class="p-title small">Мы дороже. И вот за что.</div>';
  for (const [title, text] of DATA.pricier) {
    html += `<div class="pricier-point"><b>${title}</b> ${text}</div>`;
  }
  html += '<div class="p-spacer"></div>';
  html += footerBlock();
  html += '</div></section>';
...
function buildAllScreens() {
  const parts = [];
  parts.push({id:'cover', html: screenCover()});
  parts.push({id:'tri-shaga', html: screenThreeSteps()});
  parts.push({id:'navigator', html: screenNavigator()});
  parts.push({id:'derevo-metall', html: screenWoodOrMetal()});
  parts.push({id:'my-dorozhe', html: screenWeArePricier()});
  for (const d of DATA.dividers) parts.push({id:`razdel-${d.accent}`, html: dividerScreen(d)});
  for (const fam of DATA.families) {
    parts.push({id:`fam-${fam.num}-visual`, html: familyVisual(fam)});
    parts.push({id:`fam-${fam.num}-data`, html: familyData(fam)});
  }
  return parts;
}

(function render() {
  let parts = buildAllScreens();
  const only = new URLSearchParams(location.search).get('only');
  if (only) parts = parts.filter(p => p.id === only);
  document.getElementById('screens').innerHTML = parts.map(p => p.html).join('');
})();
'''
...
{JS}
    ...
    with open("index.html", "w", encoding="utf-8") as f:
        f.write(html)
    n = 5 + len(DATA["dividers"]) + 2 * len(DATA["families"])
